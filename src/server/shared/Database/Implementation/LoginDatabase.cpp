...
void DoPrepareStatements(MySQLConnection &conn)
{
	...
	conn.prepareStatement(LOGIN_INS_CHAR_SPELL, "INSERT INTO account_spell (accountId, spell, active, disabled) VALUES (?, ?, ?, ?)");
    conn.prepareStatement(LOGIN_SEL_CHARACTER_SPELL, "SELECT spell, active, disabled FROM account_spell WHERE accountId = ?");
    conn.prepareStatement(LOGIN_DEL_CHAR_SPELL_BY_SPELL, "DELETE FROM account_spell WHERE spell = ? AND accountId = ?");
    conn.prepareStatement(LOGIN_DEL_CHAR_SPELL, "DELETE FROM account_spell WHERE accountId = ?");
}
