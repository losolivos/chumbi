...
public:
    ...
    {
        boss_skarvald_the_constructorAI(Creature* creature) : BossAI(creature, DATA_SKARVALD_DALRONN_EVENT) { }
        ...
        void JustDied(Unit* killer) override
        {
            if (!ghost)
            {
                if (Creature* dalronn = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_DALRONN)))
                {
                    ...
                    {
                        dalronn->SetFlag(OBJECT_FIELD_DYNAMIC_FLAGS, UNIT_DYNFLAG_LOOTABLE);
                        dalronn->AI()->DoAction(0);
                        _JustDied();
                        Talk(YELL_SKARVALD_DAL_DIED);
                        // Reward currency - need to do manual way or otherway its exploitable by doing half-encounter
                        if (auto player = killer->GetCharmerOrOwnerPlayerOrPlayerItself())
                        {
                            if (auto group = player->GetGroup())
                            {
                                for (auto itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
                                {
                                    ...
                                    player->ModifyCurrency(395, 24 * CURRENCY_PRECISION);
                                }
                            }
                            else
                                player->ModifyCurrency(395, 24 * CURRENCY_PRECISION);
...
public:
    ...
    {
        boss_dalronn_the_controllerAI(Creature* creature) : BossAI(creature, DATA_SKARVALD_DALRONN_EVENT) { }
        ...
        void JustDied(Unit* killer) override
        {
            if (!ghost)
            {
                if (Creature* skarvald = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_SKARVALD)))
                {
                    ...
                    {
                        skarvald->SetFlag(OBJECT_FIELD_DYNAMIC_FLAGS, UNIT_DYNFLAG_LOOTABLE);
