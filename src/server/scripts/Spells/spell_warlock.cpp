                ...
                if (GetCaster())
                    if (GetTarget())
                        if (Aura *agony = GetTarget()->GetAura(aurEff->GetSpellInfo()->Id, GetCaster()->GetGUID()))
                        {
                            ...
                            RecalculateDamage(agony->GetStackAmount());
                        }
            }

            void RecalculateDamage(uint8 _count)
            {
                _tp->SetHitDamage(uint32(384 * _count * (0.36f* _tp->GetHitUnit()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_SHADOW))));
            }
