...
public:
    ...
    enum eEvents
    {
        ...
        EVENT_DONE              = 6,
        EVENT_TALK_DEATH        = 7,
        EVENT_SUMMON_TRIGGERS   = 8,
        EVENT_BUBBLE_BRUST      = 9
    };
    ...
    struct boss_wise_mari_AI : public BossAI
    {
        ...
        {        
            phase = 1;
            if (Unit* player = me->FindNearestPlayer(20.0f))
            {
            ...
            if (uint32 eventId = cosmeticEvents.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_DONE:
                    {
                        ...
                        me->CastSpell(me, SPELL_QUIET_SUICIDE, false, NULL, NULL, SelectTarget(SELECT_TARGET_RANDOM)->GetGUID());
                        break;
                    }
