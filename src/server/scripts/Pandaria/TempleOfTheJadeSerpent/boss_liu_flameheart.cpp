    ...
    struct boss_liu_flameheart_AI : public BossAI
    {
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            if (me->GetHealthPct() <= 30.f && status == PHASE_2)
            {
                ...
                Talk(TALK_EVENT_2);
                events.Reset();
                events.ScheduleEvent(EVENT_SUMMON_YULON, 1000);
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch(eventId)
                {
                    ...
                    case EVENT_SUMMON_YULON:
                        DoCastAOE(SPELL_SUMMON_JADE_SERPENT);
                        DoCastAOE(SPELL_JADE_SOUL);
            ...
            if (me->GetHealth() <= damage)
            {
                ...
                if (!leaveEvent)
                {
                    ...
                    if (Creature* liu = Unit::GetCreature(*me, instance->GetData64(DATA_LIU)))
                    {
                        ...
                        liu->SetReactState(REACT_AGGRESSIVE);
                        liu->ClearUnitState(UNIT_STATE_ROOT);
                        liu->DealDamage(liu, liu->GetHealth(), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
                    }
                }
            }
        }
