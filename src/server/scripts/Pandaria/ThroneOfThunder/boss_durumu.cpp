    ...
    public:
        ...
        struct boss_durumuAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                events.ScheduleEvent(EVENT_CHECK_FOGS, 1500);
            }
            ...
            void SummonBeamsEye()
            {
                ...
                GetPlayerListInGrid(list, me, 100.0f);

                if (!list.empty())
                    if (Creature* eyeStatic = me->SummonCreature(RED_EYE,me->GetPositionX(), me->GetPositionY(),me->GetPositionZ()))
                        if (Player* target = Trinity::Containers::SelectRandomContainerElement(list))
                        {
                            eyeStatic->CastSpell(target, SPELL_RED_BEAM, true);
                            list.remove(target);
                        }
                        
                if (!list.empty()) // Check again coz we change player list before
                    if (Creature* eyeStatic = me->SummonCreature(BLUE_EYE, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ()))
                        if (Player* target = Trinity::Containers::SelectRandomContainerElement(list))
                            eyeStatic->CastSpell(target, SPELL_BLUE_BEAM, true);
            }
            ...
            { 
                ...
                uint32 _event = events.ExecuteEvent();
                switch (_event)
                {
                ...
                switch (_event)
                {
                    ...
                    case EVENT_LIGHT_SPECTRUM_SAY_SUMMON:
                        ...
                        for (uint8 x = 0; x <= 3; x++)
                        {
                            me->GetRandomPoint(me->GetPosition(), 50.0f, pos);
                            me->CastSpell(pos, SPELL_SUMMON_CRIMSON_FOG, true);
                        }

                        me->GetRandomPoint(me->GetPosition(), 50.0f, pos);
                        me->CastSpell(pos, SPELL_SUMMON_BLUE_FOG, true);
            ...
            void UpdateOrientation()
            {
                ...
                uint64 tarGuid = 0;
                if (Unit* Beam = me->GetFirstMinionByEntry(YELLOW_EYE_MOVER))
            ...
            void EndBeamPhase()
            {
                ...
                if (Unit* eye = me->GetFirstMinionByEntry(HUNGRY_EYE))
            ...
            void DoAction(const int32 action)
            {
                switch (action)
                {
                    ...
                    case ACTION_BEGIN_INTRO: // When players kill roaming
                        ...
                        events.ScheduleEvent(EVENT_UNLOCK_AI, 21000);
                        break;
