...
class boss_rook_stonetoe : public CreatureScript
{
    public:
        ...
        {
            boss_rook_stonetoeAI(Creature* creature) : BossAI(creature, DATA_FALLEN_PROTECTORS), summons(me)
            {
            ...
            void FinishEvent()
            {
                ...
                if (instance)
                    instance->SetData(DATA_FALLEN_PROTECTORS, DONE);
            }
...
class boss_he_softfoot : public CreatureScript
{
    public:
        ...
        {
            boss_he_softfootAI(Creature* creature) : BossAI(creature, DATA_FALLEN_PROTECTORS), summons(me)
            {
                instance  = creature->GetInstanceScript();
            }
...
class boss_sun_tenderheart : public CreatureScript
{
    public:
        ...
        {
            boss_sun_tenderheartAI(Creature* creature) : BossAI(creature, DATA_FALLEN_PROTECTORS), summons(me)
            {
                instance  = creature->GetInstanceScript();
            }
    ...
    public:
        ...
        class spell_rook_corrupted_brew_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                std::list<Unit*> targets = caster->ToCreature()->AI()->SelectTargetList( count, SELECT_TARGET_RANDOM, 100.0f, true);
                if (!targets.empty())
        ...
        class spell_sun_shadow_word_bane_main_AuraScript : public AuraScript
        {
            ...
            void OnPeriodic(AuraEffect const* aurEff)
            {
                ...
                if (aurEff->GetTickNumber() <= 3)
                    if (Creature* Sun = target->FindNearestCreature(NPC_SUN_TENDERHEART, 300.0f, true))
