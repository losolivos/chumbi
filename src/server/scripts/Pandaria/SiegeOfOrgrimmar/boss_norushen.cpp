    ...
    public:
        ...
        struct boss_norushenAI : public ScriptedAI
        {
            ...
            void Reset()
            {
                ...
                if (instance)
                    instance->SetData(DATA_NORUSHEN, NOT_STARTED);
            ...
            void JustSummoned(Creature* summon)
            {
                ...
                if (summon->GetEntry() != NPC_AMALGAM_OF_CORRUPTION)
                {
            ...
            void EnterEvadeMode()
            {
	            ...
	            if (instance)
                {
                    instance->SetData(DATA_NORUSHEN, FAIL);
                    instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_CORRUPTION_BAR);
                    if (Creature* amalgam = me->FindNearestCreature(NPC_AMALGAM_OF_CORRUPTION, 300.0f, true))
                ...
                while(uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_ENCOUNTER_1:
	                        ...
	                        if (instance)
                            {
                                instance->SetData(DATA_NORUSHEN, IN_PROGRESS);
                        ...
                        case EVENT_START_OUTRO:
                            if (instance)
                                instance->SetData(DATA_NORUSHEN, DONE);
                        ...
                        case EVENT_SPAWN_NORUSHEN_PRIDE:
                            me->SummonCreature(NPC_NORUSHEN, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(), TEMPSUMMON_MANUAL_DESPAWN);
        ...
        bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            if (CAST_AI(boss_norushen::boss_norushenAI, creature->AI())->introDone == true &&
                ...
                creature->GetInstanceScript() && creature->GetInstanceScript()->GetData(DATA_NORUSHEN) != DONE) // Opening gossip.
...
class boss_amalgam_of_corruption : public CreatureScript
{
    public:
        ...
        {
            boss_amalgam_of_corruptionAI(Creature* creature) : BossAI(creature, DATA_NORUSHEN), summons(me)
            {
                instance = creature->GetInstanceScript();
            }
            ...
            void KilledUnit(Unit* victim)
            {
                if (victim->GetTypeId() == TYPEID_PLAYER)
                    if (Creature* norushen = me->FindNearestCreature(NPC_NORUSHEN, 300.0f, true))
        ...
        struct npc_manifestation_of_corruptionAI : public ScriptedAI
        {
            ...
            void IsSummonedBy(Unit* /*summoner*/)
            {
                ...
                else // Foul Link handling.
                {
                    ...
                    if (Creature* amalgam = me->FindNearestCreature(NPC_AMALGAM_OF_CORRUPTION, 200.0f, true))
            ...
            void JustDied(Unit* killer)
            {
                if (me->GetEntry() == NPC_MANIFEST_OF_CORRUPTION_L)
                {
                    ...
                    if (Creature* quarantine = me->FindNearestCreature(NPC_QUARANTINE_MEASURES, 200.0f, true))
                        if (Creature* amalgam = quarantine->FindNearestCreature(NPC_AMALGAM_OF_CORRUPTION, 200.0f, true))
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            if (Creature* amalgam = me->FindNearestCreature(NPC_AMALGAM_OF_CORRUPTION, 150.0f, true))
                            {
    ...
    public:
        ...
        struct npc_purifying_light_orbAI : public ScriptedAI
        {
            ...
            void OnSpellClick(Unit* clicker)
            {
                ...
                if (!clicker->ToPlayer()->GetActiveSpec() || !clicker->FindNearestCreature(NPC_AMALGAM_OF_CORRUPTION, 300.0f, true))
                    return;

                // Retrieve, check and set realms available.
                Creature* amalgam = clicker->FindNearestCreature(NPC_AMALGAM_OF_CORRUPTION, 300.0f, true);
            ...
            void MovementInform(uint32 type, uint32 pointId)
            {
                ...
                switch (pointId)
                {
                    ...
                    case POINT_CHO_MOVE_5: // Nothing to do here.
			            if (Creature* norushen = me->FindNearestCreature(NPC_NORUSHEN, 500.0f, true))
                        {
                            if (CAST_AI(boss_norushen::boss_norushenAI, norushen->AI())->introStarted == false &&
                                ...
                                instance && instance->GetData(DATA_NORUSHEN) != DONE)
                            {
...
class at_soo_vault_of_yshaarj_entrance : public AreaTriggerScript
{
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* /*areaTrigger*/)
        {
			...
			if (Creature* norushen = player->FindNearestCreature(NPC_NORUSHEN, 500.0f, true))
            {
    ...
    public:
        ...
        class spell_amalgam_corruption_bar_AuraScript : public AuraScript
        {
            ...
            void OnTick(AuraEffect const* aurEff)
            {
                ...
                Creature* amalgam = target->FindNearestCreature(NPC_AMALGAM_OF_CORRUPTION, 500.0f, true);
                if (!amalgam)
                    return;
