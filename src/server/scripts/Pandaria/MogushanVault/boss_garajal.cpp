        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    {
                        if (_count_sec_attack == 4)
                            _count_sec_attack = 0;

                        switch (_count_sec_attack)
                        {
                        case 0:
                            me->CastSpell(SelectTarget(SELECT_TARGET_TOPAGGRO), SPELL_RIGHT_CROSS, false);
                            _count_sec_attack++;
                            break;
                        case 1:
                            me->CastSpell(SelectTarget(SELECT_TARGET_TOPAGGRO), SPELL_LEFT_HOOK, false);
                            _count_sec_attack++;
                            break;
                        case 2:
                            me->CastSpell(SelectTarget(SELECT_TARGET_TOPAGGRO), SPELL_HAMMER_FIST, false);
                            _count_sec_attack++;
                            break;
                        case 3:
                            me->CastSpell(SelectTarget(SELECT_TARGET_TOPAGGRO), SPELL_SWEEPING_KICK, false);
                            _count_sec_attack++;
                            break;
                        }
                        events.ScheduleEvent(EVENT_SECONDARY_ATTACK, urand(5000, 10000));
