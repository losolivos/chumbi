...
public:
    ...
    struct boss_tayakAI : public BossAI
    {
        boss_tayakAI(Creature* creature) : BossAI(creature, DATA_TAYAK), summons(me)
        {
            ...
            me->CastSpell(me, SPELL_VISINTRO_TAYAK);
        }
        ...
        void EnterCombat(Unit* /*who*/)
        {
            if (pInstance)
            {
                if (!pInstance->CheckRequiredBosses(DATA_TAYAK))
                {
                    EnterEvadeMode();
                    return;
                }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (!_CanStart)
            {
                if (instance->GetBossState(DATA_TAYAK) == NOT_STARTED || instance->GetBossState(DATA_TAYAK) == TO_BE_DECIDED)
                {
                    Creature* Silentwing = GetClosestCreatureWithEntry(me, NPC_KORTHIK_SILENTWING, 200.0f, true);
                    Creature* Galeslicer = GetClosestCreatureWithEntry(me, NPC_SETTHIK_GALESLICER, 200.0f, true);
                    Creature* Klithak = GetClosestCreatureWithEntry(me, NPC_INSTRUCTOR_KLITHAK, 200.0f, true);
                    Creature* Takthok = GetClosestCreatureWithEntry(me, NPC_INSTRUCTOR_TAKTHOK, 200.0f, true);
                    Creature* Swarmer = GetClosestCreatureWithEntry(me, NPC_KORTHIK_SWARMER, 200.0f, true);
                    Creature* Tempest = GetClosestCreatureWithEntry(me, NPC_SETTHIK_TEMPEST, 200.0f, true);

                    if (!Silentwing&& !Galeslicer && !Klithak && !Takthok && !Swarmer && !Tempest)
                    {
                        me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                        me->SetReactState(REACT_AGGRESSIVE);
                        if (me->HasAura(128788))
                            me->RemoveAura(128788);
                        _CanStart = true;
                        return;
                    }
                }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_UNSEEN_STRIKE:
                {
                    ...
                    if (UnseenTarget = SelectTarget(SELECT_TARGET_RANDOM))
                    {
                        Talk(ANN_UNSEEN, UnseenTarget->GetGUID());
                        me->CastSpell(UnseenTarget, SPELL_UNSEEN_STRIKE_TR, true);
                        me->AddAura(SPELL_UNSEEN_STRIKE_MKR, UnseenTarget);
                        me->GetMotionMaster()->MoveChase(UnseenTarget);
                        me->SetReactState(REACT_PASSIVE);
                        unseenReturn = true;
                    }
                    events.ScheduleEvent(EVENT_UNSEEN_DAMAGE,4000);
                    events.ScheduleEvent(EVENT_UNSEEN_STRIKE_RETURN, 5500);
                    events.ScheduleEvent(EVENT_UNSEEN_STRIKE, urand(53000, 61000));
                    break;
                }
                case EVENT_UNSEEN_DAMAGE:
                    if (UnseenTarget)
                        me->CastSpell(UnseenTarget, SPELL_UNSEEN_DAMAGE, false);
                    break;
                ...
                case EVENT_TAYAK_WIND_STEP:
                {
                    ...
                    if (Unit* target = SelectTarget(SELECT_TARGET_FARTHEST, 0, 50.0f, true))
                    {
                        ...
                        me->AddAura(SPELL_WIND_STEP_B_DMG, target);
                        Map::PlayerList const &PlayerList = me->GetMap()->GetPlayers();
                        if (!PlayerList.isEmpty())
                        {
                            ...
                            {
                                if (Player* playr = i->GetSource())
                                    if (me->GetDistance2d(playr) <= 8 && playr->GetGUID()!=target->GetGUID())
                                        me->AddAura(SPELL_WIND_STEP_B_DMG, playr);
                            }
                        }
                    }
                ...
                case EVENT_STORM_UNLEASHED:
                {
                    ...
                    if (Player* player = ObjectAccessor::GetPlayer(*me, plGuid))
                        if (Creature* storm = me->SummonCreature(NPC_US_TORNADO, PlayerTelePos))
                            storm->AI()->SetData(TYPE_NEW_STORMID, 1);
                ...
                case EVENT_SUMMON_TORNADOES:
                {
                    ...
                    if (Creature* storm = me->SummonCreature(NPC_US_TORNADO, !storm1Done ? Tornado1[choice] : Tornado2[choice]))
                        storm->AI()->SetData(TYPE_NEW_STORMID, 3 * storm1Done + choice);
    ...
    struct npc_tempest_slash_tornadoAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (Creature* tayak = pInstance->instance->GetCreature(pInstance->GetData64(NPC_TAYAK)))
                if (tayak->AI()->GetData(TYPE_CURRENT_STORMID) == PHASE_STORM_UNLEASHED)
