...
class npc_shang_xi_choose_faction : public CreatureScript
{
    public:
        ...
        bool OnGossipSelect(Player* player, Creature* /*creature*/, uint32 /*sender*/, uint32 action)
        {
            ...
            {
                player->SetByteValue(UNIT_FIELD_BYTES_0, 0, RACE_PANDAREN_ALLI);
                player->setORace();
                player->setFactionForRace(RACE_PANDAREN_ALLI);
                player->SaveToDB();
                WorldLocation location(0, -8866.55f, 671.93f, 97.90f, 5.31f);
                player->TeleportTo(location);
                player->SetHomebind(location, 9);
                player->learnSpell(668, false); // common language
                player->learnSpell(108127, false); // pandaren language
            }
            else if (action == GOSSIP_ACTION_INFO_DEF + 3)
            {
                ...
                player->SaveToDB();
                WorldLocation location(1, 1577.30f, -4453.64f, 15.68f, 1.84f);
                player->TeleportTo(location);
                player->SetHomebind(location, 363);
                player->learnSpell(669, false); // common language
                player->learnSpell(108127, false); // pandaren language
