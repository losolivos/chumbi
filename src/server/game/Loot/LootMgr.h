...
class LootTemplate
{
    ...
    public:
        ...
        void ProcessPersonalItem(Loot& loot, uint32 specialisation, Player* player) const;
        void CopyConditions(ConditionList const &conditions);
        void CopyConditions(LootItem *li) const;

        LootItem* GeneratePersonalLoot(Player* player, uint32 difficulty, uint16 lootMode) const; 
...
struct Loot
{
    ...
    bool FillSharedLoot(uint32 lootId, LootStore const& store, std::list<uint64> looters, Creature* creature, bool noEmptyError = false, uint16 lootMode = LOOT_MODE_DEFAULT);
    ...
    uint32 Merge(Loot* other, Player* source);
