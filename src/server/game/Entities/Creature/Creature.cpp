...
void Creature::Update(uint32 diff)
{
    ...
    switch (m_deathState)
    {
        ...
        case ALIVE:
        {
            ...
            {
                if (!IsInEvadeMode() && (!bInCombat || IsPolymorphed()) && !HealthRegenIsDisable()) // regenerate health if not in combat or if polymorphed
                    RegenerateHealth();
...
bool Creature::isTagShared() const
{
    return ((m_creatureInfo->type_flags & CREATURE_TYPEFLAGS_TAG_SHARED) && !GetMap()->IsRaidOrDungeon());
}
...
void Creature::SetLootRecipient(Unit* unit)
{
    ...
    if (isTagShared())
    {
        ...
        if (!isTappedBy(player))
            m_lootRecipientList.push_back(player->GetGUID());
...
{
    if (isTagShared())
        return std::find(m_lootRecipientList.begin(), m_lootRecipientList.end(), player->GetGUID()) != m_lootRecipientList.end();
    else
    {
...
{ 
    if (isTagShared())
        return !m_lootRecipientList.empty();
    else
        return m_lootRecipient || m_lootRecipientGroup;
}

void Creature::sendPersonalRewards()
{
    if (m_lootRecipientList.empty())
        return;

    for (std::list<uint64>::const_iterator i = m_lootRecipientList.begin(), end = m_lootRecipientList.end(); i != end; ++i)
    {
        Player *player = ObjectAccessor::FindPlayer(*i);
        if (!player)
            continue;

        ObjectGuid lootObjectGuid = GetGUID();
        ObjectGuid looterGuid = player->GetGUID();
        ObjectGuid killerGuid = player->GetGUID();
        ObjectGuid victimGuid = GetGUID();

        WorldPacket data(SMSG_LOOT_LIST);
        data.WriteBitSeq<4, 5, 2, 3, 7>(lootObjectGuid);
        data.WriteBit(looterGuid != 0);
        if (looterGuid)
            data.WriteBitSeq<5, 1, 3, 0, 7, 6, 2, 4>(looterGuid);

        data.WriteBit(killerGuid != 0);
        if (killerGuid)
            data.WriteBitSeq<6, 4, 0, 2, 7, 5, 1, 3>(killerGuid);

        data.FlushBits();
        data.WriteBitSeq<1, 6, 0>(lootObjectGuid);

        if (killerGuid)
            data.WriteByteSeq<4, 7, 5, 1, 2, 6, 3, 0>(killerGuid);

        if (looterGuid)
            data.WriteByteSeq<3, 2, 7, 0, 5, 1, 6, 4>(looterGuid);

        data.WriteByteSeq<0, 2, 5, 4, 3, 1, 6, 7>(lootObjectGuid);

        player->SendMessageToSet(&data, true);

        player->KilledMonsterCredit(GetEntry(), GetGUID());
    }
    ...
    if (CreatureTemplate const* cInfo = sObjectMgr->GetCreatureTemplate(GetEntry()))
        if (uint32 lootid = cInfo->lootid)
            loot.FillSharedLoot(lootid, LootTemplates_Creature, m_lootRecipientList, this, false, GetLootMode());
}
